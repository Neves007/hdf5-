...
class DataHandler():
    ...
    def save(self):
        ...
        # 使用Dao对象保存数据集和元数据
        dataset = self.convert_to_storage_format(self.dataset)
        self.dao.set_dataset(dataset)
    ...
    def load(self):
        ...
        self.dataset = self.dao.get_dataset()  # 从Dao对象加载数据集
        self.metadata = self.dao.get_metadata()  # 从Dao对象加载元数据
        self.meta_reflection()  # 进行元反射
