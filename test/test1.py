...
class EpochAnalyzeManager:
    ...
    def analyze_epoch(self, model_name, epoch_data):
        ...
        """
        # 这里模拟对每个 epoch 数据的分析
        print(f"Analyzing epoch {epoch_data['epoch']} for model {model_name}: Accuracy={epoch_data['accuracy']}")
        # 分析完毕后通知订阅者
        self.notify(model_name, epoch_data)

class ModelAnalyzeManager(Observer):
    """
    汇总和分析每个模型的所有 epoch 结果，找出最佳 epoch，并分析时间演化趋势。
    """
    def __init__(self):
        self.model_best_epochs = {}  # 存储每个模型的最佳 epoch 数据
        self.subscribers = []  # 存储所有的观察者（订阅者）
        self.model_epochs_data = {}  # 存储每个模型的所有 epoch 数据
    ...
    def update(self, data):
        ...
        if len(self.model_epochs_data[model_name]) == self.epochs:
            ...
            print(f"Model {model_name} analysis complete. Best Epoch: {best_epoch_data['epoch']}, Accuracy: {best_epoch_data['accuracy']}")
            # 通知订阅者当前模型的最佳 epoch 数据
...
class OverallAnalyzeManager(Observer):
    ...
    def __init__(self):
        ...
        self.best_epoch_data = None  # 最优模型的最佳 epoch 数据
    ...
    def update(self, data):
        ...
        if len(self.models_best_results) == len(self.models):
            ...
            print(f"Updated overall best model: Model={self.best_model}, Epoch={self.best_epoch_data['epoch']}, Accuracy={self.best_epoch_data['accuracy']}")
...
class AnalyzeManagerManager():
    ...
    def analyze(self, models: List[str], epochs: int):
        """
        对指定的模型和 epoch 数量执行分析。
        :param models: 模型名称的列表
        :param epochs: 每个模型的 epoch 数量
        """
        self.models = models  # 存储模型列表
        self.epochs = epochs  # 存储 epoch 数量
        self.model_analyze_manager.epochs = epochs  # 设置模型分析管理器的 epoch 数量
...
class DummyExpGenerator:
    ...
    def generate_epoch_data(self, model_name, epoch):
        ...
        :param epoch: 当前 epoch 的编号
        :return: 模拟的 epoch 数据，包括 epoch 编号和 accuracy
        """
        return {"epoch": epoch, "accuracy": 0.8 + 0.01 * epoch} # 假设每个 epoch 的 accuracy 提高 0.01
